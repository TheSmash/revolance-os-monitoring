...
public class EdenSpaceUsage implements ReduceOperator
{
    ...
    public Series apply(Series series, long since)
    {
        ...
        {
            usage.addSample(date, series.getSerie(String.valueOf(JstatCommand.Column.EU)).at(date));
            maxUsage.addSample(date, series.getSerie(String.valueOf(JstatCommand.Column.EC)).at(date));
        }
